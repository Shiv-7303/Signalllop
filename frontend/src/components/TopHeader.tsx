...
import { Sparkles, Bell, Menu, Zap } from 'lucide-react'
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-6">
          <Badge className={cn(
            ...
            user?.plan === 'starter' ? "bg-brand-blue/10 text-brand-blue" :
          ...
          <div className="flex items-center gap-3">
            <div className="w-32 bg-white/5 rounded-full overflow-hidden h-1.5 border border-white/5">
              <div 
                 className="h-full bg-brand-blue rounded-full shadow-[0_0_10px_rgba(64,150,255,0.8)]" 
      ...
      <div className="flex items-center gap-4">
        {user?.plan !== 'pro' && (
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={springConfig15}>
            <Button 
              size="sm" 
              className="bg-brand-blue hover:bg-brand-blue/90 text-white gap-2 font-bold rounded-full px-5 shadow-lg shadow-brand-blue/20 border border-white/10 transition-colors"
