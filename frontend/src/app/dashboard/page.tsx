    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center space-y-8 bg-slate-900/40 backdrop-blur-2xl rounded-[3rem] border border-white/10 shadow-premium">
        ...
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={springConfig15}>
          <Button asChild className="bg-brand-orange hover:bg-brand-orange/90 text-white rounded-full px-8 h-12 font-bold shadow-xl shadow-brand-orange/20 border border-white/10">
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="flex gap-3">
           ...
           <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={springConfig15}>
             <Button asChild className="bg-brand-orange hover:bg-brand-orange/90 text-white rounded-full px-6 h-11 font-bold shadow-lg shadow-brand-orange/20 border border-white/10">
        ...
        <div className="lg:col-span-2 p-2 bg-white/5 backdrop-blur-2xl rounded-[3rem] border border-white/10 shadow-premium flex flex-col">
           <div className="bg-slate-900/80 rounded-[2.5rem] p-8 h-full flex flex-col shadow-inner border border-white/5 space-y-6">
              <div className="flex justify-between items-center">
                 ...
                 <Button variant="ghost" size="sm" className="text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-brand-orange hover:bg-white/5 h-6 transition-colors">Explore all</Button>
              </div>
                ...
                {reportData?.best_communities?.slice(0, 4).map((community: any, i: number) => (
                  ...
                  >
                    <div className="flex justify-between items-start relative z-10">
                      <Badge className="bg-brand-orange/20 text-brand-orange border border-brand-orange/30 font-bold rounded-lg px-3 py-1 backdrop-blur-md">r/{community.subreddit}</Badge>
                    ...
                    <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-brand-orange/10 rounded-full blur-2xl group-hover:bg-brand-orange/30 transition-colors duration-500" />
                  </motion.div>
                ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {opportunities?.slice(0, 3).map((opt: any, i: number) => {
            ...
            return (
              ...
              >
                <Card className="bg-slate-900/60 backdrop-blur-2xl border border-white/10 shadow-premium rounded-[2.5rem] h-full flex flex-col hover:border-brand-orange/40 transition-colors cursor-pointer overflow-hidden">
                  <CardHeader className="pb-3 space-y-4 p-8">
                    ...
                    <CardTitle className="text-lg font-bold leading-snug text-white group-hover:text-brand-orange transition-colors line-clamp-2 drop-shadow-sm">{opt.title}</CardTitle>
                  </CardHeader>
                  ...
                  <div className="absolute bottom-6 left-6 right-6 flex justify-between items-center bg-black/40 backdrop-blur-xl p-3 rounded-2xl border border-white/10">
                    ...
                    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={springConfig15}>
                      <Button asChild variant="ghost" size="sm" className="h-8 text-xs font-bold text-brand-orange hover:text-brand-orange p-0 px-4 hover:bg-white/10 rounded-xl transition-colors">
